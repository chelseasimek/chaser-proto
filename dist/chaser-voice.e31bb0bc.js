...
window.onload = function () {
  ...
  window.step = 0; // Checks if SpeechRecognition exsists in the current browser

  var SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition || window.mozSpeechRecognition || window.msSpeechRecognition || window.oSpeechRecognition; // If SpeechRecognition exsists, create a new class with options
  ...
  if (SpeechRecognition !== undefined) {
    ...
    window.recognition.onresult = updateTranscript.bind(_this);
  ...
  function updateTranscript(event) {
    ...
    for (var i = event.resultIndex; i < event.results.length; ++i) {
      ...
      if (event.results[i].isFinal && event.results[i][0].confidence > 0) {
        ...
        finalTranscript = event.results[i][0].transcript.toLowerCase(); // add final transcript to redux to be used in other components
  ...
  function evaluateTranscript(finalTranscript) {
    var understands = true;
    var returnStr = '';

    if (window.steps[window.step].includeAll) {
      window.steps[window.step].transcriptInclusions.forEach(function (str) {
        if (understands) {
          understands = finalTranscript.includes(str);
        }
      });
    } else {
      for (var i = 0; i < window.steps[window.step].transcriptInclusions.length; i++) {
        var str = window.steps[window.step].transcriptInclusions[i];
        understands = finalTranscript.includes(str);
    ...
    if (understands) {
      ...
      var suffix = window.step ? window.steps[window.step - 1].response : 'Please try again.';
...
//# sourceMappingURL=/chaser-voice.e31bb0bc.js.map